...
{
    if (isirStudentIrsRequestFlag == "02" && hsVerificationPass)
    {
        return true;
    }
    if (hasVerificationWorksheetIndStudent && (hasTaxTranscriptStudent || has1040xStudent || hasForeignTaxTranscriptStudent) && hsVerificationPass)
    {
        return true;
    }
}
...
{
    if (hasIrsExtensionStudent && !hasTaxTranscriptStudent && !has1040xStudent && !hasForeignTaxTranscriptStudent && !hsVerificationPass)
    {
        return false;
    }
    ...
    {
        if (hasVerificationWorksheetIndStudent && hasNonFilingStudent && (hasW2Student || has1099GStudent || hasSelfEmploymentStatementStudent) && (hasW2Spouse || has1099GSpouse || hasSelfEmploymentStatementSpouse) && hsVerificationPass)
        {
            if (totalIncome >= filingThreshold && (hasTaxTranscriptStudent || has1040xStudent ||  hasForeignTaxTranscriptStudent))
            {
